 ...
 69c:	89 d1                	mov    %edx,%ecx
{
 ...
 6c9:	0f b6 92 8c 0b 00 00 	movzbl 0xb8c(%edx),%edx
 ...
 749:	8b 5d 0c             	mov    0xc(%ebp),%ebx
{
 ...
 7b9:	ff 24 85 34 0b 00 00 	jmp    *0xb34(,%eax,4)
        putc(fd, c);
      }
      state = 0;
    }
 ...
 8c7:	90                   	nop
          s = "(null)";
 8c8:	ba 2a 0b 00 00       	mov    $0xb2a,%edx
        while(*s != 0){
...
free(void *ap)
{
 ...
 8e1:	a1 e0 13 00 00       	mov    0x13e0,%eax
{
 ...
 921:	5b                   	pop    %ebx
  freep = p;
 922:	89 15 e0 13 00 00    	mov    %edx,0x13e0
}
 928:	5e                   	pop    %esi
 929:	5f                   	pop    %edi
 92a:	5d                   	pop    %ebp
 ...
 959:	03 43 fc             	add    -0x4(%ebx),%eax
  freep = p;
 95c:	89 15 e0 13 00 00    	mov    %edx,0x13e0
    p->s.size += bp->s.size;
...
malloc(uint nbytes)
{
 ...
 979:	8b 45 08             	mov    0x8(%ebp),%eax
  if((prevp = freep) == 0){
 97c:	8b 3d e0 13 00 00    	mov    0x13e0,%edi
  nunits = (nbytes + sizeof(Header) - 1)/sizeof(Header) + 1;
 ...
 9bf:	73 4f                	jae    a10 <malloc+0xa0>
    ...
    if(p == freep)
 9c1:	8b 3d e0 13 00 00    	mov    0x13e0,%edi
 ...
 9ea:	e8 f1 fe ff ff       	call   8e0 <free>
  return freep;
 9ef:	8b 15 e0 13 00 00    	mov    0x13e0,%edx
 ...
 a1c:	89 70 04             	mov    %esi,0x4(%eax)
      freep = prevp;
 a1f:	89 15 e0 13 00 00    	mov    %edx,0x13e0
}
 ...
 a2f:	c3                   	ret    
    base.s.ptr = freep = prevp = &base;
 a30:	c7 05 e0 13 00 00 e4 	movl   $0x13e4,0x13e0
 a37:	13 00 00 
    base.s.size = 0;
 a3a:	bf e4 13 00 00       	mov    $0x13e4,%edi
    base.s.ptr = freep = prevp = &base;
 a3f:	c7 05 e4 13 00 00 e4 	movl   $0x13e4,0x13e4
 a46:	13 00 00 
  for(p = prevp->s.ptr; ; prevp = p, p = p->s.ptr){
 a49:	89 fa                	mov    %edi,%edx
    base.s.size = 0;
 a4b:	c7 05 e8 13 00 00 00 	movl   $0x0,0x13e8
 a52:	00 00 00 
    if(p->s.size >= nunits){
...
vmalloc(uint n, int flag)
{
 ...
 a79:	83 f8 01             	cmp    $0x1,%eax
 a7c:	77 3a                	ja     ab8 <vmalloc+0x48>
        printf(1, "Error flag init (vmalloc)\n");
        exit();
    }
    if(flag == VMALLOC_SIZE_BASE)
 a7e:	85 c0                	test   %eax,%eax
 a80:	74 1e                	je     aa0 <vmalloc+0x30>
        ptr = sbrk(n);
    else
        ptr = sbrkhuge(n);
 a82:	83 ec 0c             	sub    $0xc,%esp
 a85:	ff 75 08             	push   0x8(%ebp)
 a88:	e8 f6 fb ff ff       	call   683 <sbrkhuge>
 ...
 aa6:	e8 a0 fb ff ff       	call   64b <sbrk>
 ...
 ab0:	c9                   	leave  
        return 0;
 ab1:	83 f8 ff             	cmp    $0xffffffff,%eax
 ab4:	0f 44 c2             	cmove  %edx,%eax
}
 ab7:	c3                   	ret    
        printf(1, "Error flag init (vmalloc)\n");
 ab8:	50                   	push   %eax
 ab9:	50                   	push   %eax
 aba:	68 9d 0b 00 00       	push   $0xb9d
 abf:	6a 01                	push   $0x1
 ac1:	e8 7a fc ff ff       	call   740 <printf>
        exit();
 ac6:	e8 f8 fa ff ff       	call   5c3 <exit>
...
vfree(void *ptr)
{
 ...
 ad9:	85 d2                	test   %edx,%edx
 adb:	78 02                	js     adf <vfree+0xf>
 ...
 adf:	50                   	push   %eax
 ae0:	50                   	push   %eax
 ae1:	68 b8 0b 00 00       	push   $0xbb8
 ae6:	6a 01                	push   $0x1
 ae8:	e8 53 fc ff ff       	call   740 <printf>
        exit();
 aed:	e8 d1 fa ff ff       	call   5c3 <exit>
